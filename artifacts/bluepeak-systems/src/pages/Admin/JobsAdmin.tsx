...
function JobEditor({
  ...
  const [form, setForm] = useState<FormState>(() => (job ? toForm(job) : EMPTY_FORM));
  ...
  const toggleActive = () =>
    setForm((f) => ({ ...f, isActive: f.isActive === 'true' ? 'false' : 'true' }));
  ...
  const textarea = (field: string, rows = 4, placeholder = '') => (
    <div className="job-editor-field job-editor-field-full">
      <label>{field.replace(/([A-Z])/g, ' $1').trim()} <span className="opt">(one per line)</span></label>
      <textarea
        value={form[field] ?? ''}
        onChange={set(field)}
        rows={rows}
      ...
      <div className="modal-content job-editor-modal" onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <div>
            ...
            <span className="modal-position">{job ? job.slug : 'New position'}</span>
          </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="modal-body">
            ...
            <div className="job-editor-grid">
              ...
              <div className="job-editor-field">
                ...
                <input list="job-departments" value={form.department ?? ''} onChange={set('department')} required />
              ...
              <div className="job-editor-field">
                ...
                <select value={form.employmentType ?? ''} onChange={set('employmentType')} required>
                  {EMPLOYMENT_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
                </select>
              </div>
              <div className="job-editor-field">
                <label>Work arrangement <span className="req">*</span></label>
                <select value={form.workArrangement ?? ''} onChange={set('workArrangement')} required>
              ...
              <div className="job-editor-field">
                ...
                <input value={form.experience ?? ''} onChange={set('experience')} placeholder="e.g. 1–2 years" required />
              </div>
              <div className="job-editor-field">
                <label>Compensation <span className="req">*</span></label>
                <input value={form.compensation ?? ''} onChange={set('compensation')} placeholder="e.g. $700–$1,000/month" required />
              </div>
              <div className="job-editor-field">
                <label>Posted date <span className="req">*</span></label>
                <input type="date" value={form.postedDate ?? ''} onChange={set('postedDate')} required />
              </div>
              <div className="job-editor-field job-editor-toggle">
                ...
                >
                  {form.isActive === 'false' ? <EyeOff size={14} /> : <Eye size={14} />}
            ...
            <div className="job-editor-field job-editor-field-full">
              ...
              <textarea value={form.summary ?? ''} onChange={set('summary')} rows={2} required />
            </div>
            <div className="job-editor-field job-editor-field-full">
              <label>Overview <span className="req">*</span></label>
              <textarea value={form.overview ?? ''} onChange={set('overview')} rows={5} required />
            </div>
            <div className="job-editor-field job-editor-field-full">
              <label>Working hours <span className="req">*</span></label>
              <textarea value={form.workingHours ?? ''} onChange={set('workingHours')} rows={2} required />
            </div>
          ...
          <div className="modal-footer">
            <span className="modal-applied">{job ? `Last updated ${new Date(job.updatedAt).toLocaleDateString()}` : 'New position'}</span>
            <div className="modal-status-row">
              <button type="button" onClick={onClose} className="button button-sm button-outline">
                Cancel
              </button>
              <button type="submit" disabled={saving} className="button button-blue">
                {saving ? <Loader2 size={16} className="animate-spin" /> : <Briefcase size={16} />}
...
export function JobsAdmin({ token }: { token: string }) {
  ...
  const saveJob = async (payload: Record<string, unknown>) => {
    ...
    const res = await fetch(`${API_BASE}/api/admin/jobs${isNew ? '' : `/${(editorOpen as AdminJob).id}`}`, {
  ...
  const deleteJob = async (job: AdminJob) => {
    if (!window.confirm(`Delete the "${job.title}" position? This cannot be undone.`)) return;
    setBusy(job.id);
    try {
  ...
  const toggleActive = async (job: AdminJob) => {
    ...
    try {
      ...
      setJobs((prev) => prev.map((j) => (j.id === job.id ? { ...j, ...data.job } : j)));
    } catch (err) {
    ...
    <>
      <div className="admin-header-bar">
        ...
        <div className="admin-header-actions-inline">
          ...
          <button className="button button-blue" onClick={() => setEditorOpen('new')}>
            ...
            <tbody>
              {jobs.map((job) => (
                <tr key={job.id}>
                  ...
                  <td>
                    <div className="position-experience">{job.compensation}</div>
                  </td>
                  <td className="date-cell">
                    {new Date(job.postedDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
                  </td>
                  <td>
                    <span className={`status-badge ${job.isActive ? 'status-live' : 'status-hidden'}`}>
                  ...
                  <td>
                    <div className="action-buttons">
                      <button
                        ...
                        title={job.isActive ? 'Hide from careers page' : 'Publish to careers page'}
                      >
